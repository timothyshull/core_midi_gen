...
OSStatus
Core_midi_gen::load_smf(const std::string &filename, MusicSequence &sequence, MusicSequenceLoadFlags load_flags)
{
...
void Core_midi_gen::play_loop(MusicPlayer &player, AUGraph &graph, MusicTimeStamp sequence_length, bool should_print,
                              bool wait_at_end)
{
    auto wait_counter = 0;
    while (true) {
